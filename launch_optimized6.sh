#!/bin/bash
# Phase-6 launcher. Continuation of phase 5 at the SAME proven config:
# rollout-length 6,266,880 / num-minibatches 48 -> batch_size 130,560 (identical
# to phase 5). Entropy floors unchanged (clubgg 0.05 / clubgg_deep 0.06 /
# deep 0.085). New stem only (phase-5 lineage continues). Warm-starts from the
# highest optimized6_<N>.pt if any exist (watchdog relaunch), else from the
# highest optimized5_<N>.pt (initial transition; expected optimized5_115.pt).
...
setsid nohup .venv/bin/python -u scripts/train.py \
  ...
  --checkpoint checkpoints/optimized6.pt \
  >> "$LOG" 2>&1 < /dev/null &
PID=$!
disown "$PID" || true
