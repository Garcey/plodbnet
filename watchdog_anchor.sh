#!/bin/bash
# Stem-bumping watchdog for the v2 anchor<N> family. Checks every
# INTERVAL seconds; if scripts/train.py is gone (crash, OOM, pod
# maintenance), relaunches via launch_anchor.sh — which cold-starts
# anchor1 when no anchor checkpoints exist, else starts the NEXT stem
# warm-started from the newest loadable anchor checkpoint. Anneal
# floors/baselines/block position ride in the checkpoint.
#
# STOP cleanly:  touch runs/watchdog_anchor.stop
#
...
# runs/anchor<latest>.log periodically.
set -uo pipefail
cd /workspace/plodbnet

STOP=runs/watchdog_anchor.stop
WLOG=runs/watchdog_anchor.log
INTERVAL=60

SELF=$$
exec 9>runs/watchdog_anchor.lock
if ! flock -n 9; then
...
while true; do
  ...
  bash launch_anchor.sh >> "$WLOG" 2>&1
  sleep 30
  if pgrep -f 'scripts/train\.py' >/dev/null; then
