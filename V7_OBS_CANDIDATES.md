...
> **ERRATUM (review 2026-09-20 B1).** The premise below is wrong: `min_bet_total` / `max_bet_total` are NOT capped by the actor's own stack and ignore the short-shove lockout and the cover-short clamp, so deltas "recovered from the packed totals" describe a window that is frequently not the legal one (wrong on 23–36% of raise-legal nodes with per-seat stacks; block non-zero at 15–23% of nodes where Raise is illegal). As shipped since the fix, STK-2 and STK-5[2:4] use the engine's packed `min_raise` / `max_raise` (the same numbers `sizing_from_info` feeds the sizing head) behind the gate predicate (`max_raise > 0`, plus the env's cover-short dust screen). `PLO5BP_OBS_REV=1` reproduces the original values for checkpoints trained before the fix.
...
> **ERRATUM (review 2026-09-20 B1).** `D` is the engine's packed `max_raise` (the LEGAL max-raise delta), not `max_bet_total − street_commit_hero` — see the STK-2 erratum above. With the original definition `eff_hero − D` went negative whenever hero's own stack capped the raise.
